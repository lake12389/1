...
def text_to_speech(text_content, cache_id=None):
    ...
    class TTS_Ws_Param:
        def __init__(self, text):
            ...
            self.data = {"status": 2, "text": base64.b64encode(text.encode("utf-8")).decode("utf-8")}

        def create_url(self):
            try:
                now = datetime.utcnow()    # 必须使用UTC时间
                date = format_date_time(mktime(now.timetuple()))
                ...
                signature = base64.b64encode(signature_sha).decode("utf-8")
                authorization_origin = (
                    f'api_key="{APIKey}", algorithm="hmac-sha256", headers="host date request-line", signature="{signature}"'
                )
                authorization = base64.b64encode(authorization_origin.encode("utf-8")).decode("utf-8")
                params = urlencode({
                    "authorization": authorization,
    ...
    try:
        ...
        if cache_id and audio_data:
            ...
            with wave.open(cache_path, "wb") as wf:
                ...
                wf.setframerate(16000)  # 16000采样率
                wf.setcomptype("NONE", "not compressed")  # 不压缩
